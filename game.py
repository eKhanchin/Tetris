...
class Tetris(QWidget):
    ...
    def step(self, action: int):
        ...
        if action == Action.LEFT.value:
            ...
            if is_down:
                filled_lines = self.manager.clear_filled_lines()
                self.score += filled_lines * 100
                if self.manager.is_game_over():
                    print(f'Game over - finished with {self.score} score')
                    self.close()
                
                
        elif action == Action.EXIT.value:
            print(f"Stopped at {self.score} score")
            self.close()
